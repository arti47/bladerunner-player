...
Broken combatant forces a critical injury. **A Broken combatant is still a legal target** —
every attack surface lists them (marked "Broken"), since finishing someone off is how the
procedure ends. **Death saves and Stabilize are available on tracker combatants too**
(`rollCombatDeathProcedure` in `roller.js`), not only on the PC sheet, and a lethal crit on a
combatant card states its lethality and interval. A dead character is inert: `roller.js`
blocks every roll, guided play refuses to open a case, and the sheet's DECEASED banner
carries the wizard and character-switch buttons.
...
maneuver choice, D12 obstacle roll, and caught/escaped detection. In Solo Mode each side's
maneuver row carries **🎲 Roll it — the \<side\> is an NPC**, which reads the Solo Mode NPC
Chase Maneuvers table (D8) for **that side only** and renders it inline, so nothing has to be
decided that the book says the dice decide, and no note claims a move the other side never
made. **Each side is marked "Your character" or "An NPC"**: an NPC side rolls Driving and its
vehicle weapon on the vehicle's Maneuverability plus its own `NPC_SKILL_LEVEL` dice — never the
player's skills, never the player's key memory, and never pushable (Solo Mode p.010).
No ammo tracking.
...
(free key attribute & skills, D8 Chinyen); **NPC rolls are never pushed**; Case Briefing tables; Downtime Event table; Countdown Event table.
**One reset, owner ruling 2026-07-29:** the notes panel's only action is **⟲ Start a fresh
case**, which clears the entire case — every solo tab (notes, oracle log, the per-card inline
results, leads, all three checklists, the timer, the Shift counter and its once-per-Shift
markers), **both** roll logs (`brp:solo.log` and the global `brp:rolllog`, so the sheet and
Home read empty too), the Case Board, and any combat or chase left open — and opens on the
Case tab. It **never touches a character**: vitals, points, inventory, injuries and journal
entries all live on the sheet. Preferences (auto-pin, log scope) are not case data and
survive. There is no separate "clear notes" action. **A case has a beginning, a middle and an end (owner report, 2026-08-29):** the Case tab
leads with a three-step **on-ramp** when nothing is open (⚡ Open a case rolls the whole
briefing, asks for a name, and lands you on Shift); with a case open it instead shows
**Case #N — title** with the assignment, the Shift you reached, open leads and board size,
and one button back into play; and **✔ Close the case** (on Case and on Wrap) asks who did
it and how it ended, then files the case with the Shifts it took and the Promotion/Humanity
it paid, writes the summary into the notes, and keeps it in **Case files** — an archive
that survives "Start a fresh case". Closing a case and deleting one are now different acts.
**Flow (audit fixes, 2026-07-29):** a status
strip above the sub-nav carries the active character's Health/Resolve, the Shift number, the
Downtime cadence, the timer die and any banked Discovery Check on **every** panel; Scene rolls
skills/attacks/opposed rolls **in place** through `roller.js`'s modals (no navigation); the
Hypothesis Check and the award checklists **apply their points to the character** on
confirmation (`applyPoints`, floored at 0); a triggered Countdown Event is held as
`st.pendingEvent` and opens the Scene panel as an "Interruption" card until played; and the
sheet and combat tracker each carry a **← Back to the solo case** link while Solo Mode is on. Every oracle
...
**3.18 Guided play — the default way in** (`src/play.js`, owner report ×2, 2026-08-31).
It rolls through the engine, not beside it: `roller.js` exports **`skillPool(ch, skillKey)`**, which
builds the same Base Dice the sheet does — critical-injury disadvantages, Aiming (and spending it),
critical stress — and the card names what the character's state did to the roll.
Every other surface is a toolbox: it assumes you know which table to roll and what to say
about the answer. **▶ Play** is the first Solo pill and the default panel, and it assumes
you know nothing. It renders **one card at a time** — an eyebrow saying where you are, a
line or two of plain English saying what just happened, and two to four concrete choices —
and it drives the official machinery underneath: the Location tables to offer three real
places, the **Countdown Event Check** as you travel (narrated, not announced), a **Scene
Check** for how hard the place will be, real **skill rolls** (attribute die + skill die,
6+, the failed roll pushable with its cost applied), **Imagining Clues** for what you find,
**Case Table 3** for who you meet (a named person, not a trait blob), and the book's own
**Hypothesis Check** when you accuse someone — a suspect's rating starts at
`HYPOTHESIS.newRating` and upgrades one step per piece of evidence, which is the printed
procedure. Everything found is written into the case notes as it happens. It holds **no
rules and no state of its own**: the case, notes, Shift counter, timer and character live
where they already lived (`st.play` is only which question is on screen), so you can drop
into the other tabs mid-case and come back. **Every guided roll is recorded** — skill rolls,
pushes (with what the push cost), the Countdown check and the Hypothesis Check all append to
the global `RollLog` (`source: "solo"`, so the notes are not written twice), and its Shift
transition reports itself exactly as the Wrap tab's does. **It also feeds the two tracking
surfaces it drives:** every clue and named person becomes a **Case Board** box (a clue is
linked to the suspect it points at), every suspect becomes a **Leads** hypothesis carried at
the rating this panel is using, a disproved accusation removes its lead, and an investigative
success offers **🔍 Bank a Discovery Check** — so dropping into Board or Leads mid-case shows
the case you have been playing. **The briefing arrives before the naming:** ▶ Get me a case
shows what dispatch handed you (assignment, relevance, complication, hook) with *Take the
case* / *Give me a different one*, and only the acceptance asks for a name and writes the
...
| `play.js` | Guided play (§3.18) — renders ONE card at a time (where you go, what you do, what you found) and calls the same engines the tabs expose. Holds no rules and no state of its own; solo.js hands it the case, the notes and the character. |
...
| `solo.js` | Solo assistant — PRESENT (official Solo Mode). Panels: **▶ Play** (guided, the default) · Case · Shift · Scene · **Board** (house aid) · Leads · Wrap · Notes. |
...
characters/{characterId}
  ...
  state:     { health, resolve, conditions{key:true}, criticalInjuries[], criticalStress{name,text}|null,
               ...
               shiftUses{key:true}, permanentResolveLoss, dead,
               spent{pp,humanity} }                                // v5 — deliberate spends
...
`{ active, env:"foot"|"ground"|"aerial", round, distIdx, obstacle{roll,text,die}|null,
prey, pursuer, vehicles{prey,pursuer}, hull{prey,pursuer}, log[] }`. `distIdx` indexes `RANGES`; below 0 = caught, past the end = escaped.

**Case lifecycle** (local-only, §3.15). The open case lives in `brp:solo.caseOpen`:
`{ no, title, assignment, opened, character, openStats{pp,humanity} }` — `openStats` is the
...
Firebase and not part of `SCHEMA_VERSION` — it is per-device scratch state, like `brp:chase`.
...
change**. Current `SCHEMA_VERSION = 5`. **v5 `state.spent`** counts points *deliberately spent*
(a specialty, a skill step, gear bought with Promotion Points — never a penalty). A closed
case reports `(points now − points at opening) + (spent now − spent at opening)`, so a case
that funded a skill step still files as having paid for it; `caseOpen.openStats` carries the
spend counters alongside the balances.
...
10. **Audit hygiene (adopted 2026-08-29).** Every audit reports **all** findings per run,
    never stop-on-first. A clean run must state what it covered **and** what it did not, so
    "clean" is a bounded claim. When a check is disabled, skipped, or optional, the run must
    say so in its output — a silent skip reads exactly like a pass (this is why the browser
    suites print a `!! NOT RUN` banner and the count of checks that did not execute).
11. **Merge to `main` every time (owner instruction, 2026-07-28).** Work still happens on
...
### 11.1 Two different jobs (adopted 2026-08-29)

"Audit until there are no more errors" is two jobs, and conflating them is why repeated
audits never terminate:

| Job | Question | Terminates? |
|---|---|---|
| **Converge** | Is the code clean against the checks that exist? | Yes — run `npm test`, fix, repeat; stop when it passes **twice in a row with no source change between**. Do not add checks or widen scope while converging. |
| **Widen** | Is there a *kind* of fault nothing looks for yet? | No — so scope each pass to **one named lens**, build a permanent machine-checkable version of it, run it to zero, and wire it into `npm test`. |

### 11.2 Lens register — what the suite can and cannot see

Each lens is a permanent audit in `tests/`, reports **every** finding per run (never
stop-on-first), and prints its own coverage boundary on a clean run.

| Lens | Where | Covers | Does not cover |
|---|---|---|---|
| **Reachability** (2026-08-29) | `tests/reachability.test.mjs` | R1 capability→control (runtime crawl **and** a static check that every named export in `src/*.js` is referenced); R2 accessible names, no glyph-only or bare-jargon labels; R3 dialogs labelled/leaveable/Escape-closing; R4 empty states name a control on their screen; R5 no control throws or empties the screen (storage snapshotted and restored around every click); R6 the app offers a control for anything it can write; R7 cold start reaches a sheet by clicking alone; R8 every panel explains itself; R9 dialog contents. 25 surfaces. | Visual affordance; screen-reader narration of content; wording quality; tab order outside dialogs; Firebase paths; scanability of long pickers; deep multi-step flows walked to completion. |

Unused lenses, roughly by expected yield: fidelity to source · correctness of core
transforms under sequencing · the primary flow measured in interactions · hostile input ·
storage failure (quota, denied, two tabs) · migration from older records · accessibility of
content · first-run/empty states · failure messaging · performance envelope.
...
| 2026-07-28 | **Case generator showed no result (user-reported).** Root cause: `paintResults` hangs a result slot on the `.card` that owns the button, but Solo's **Core Case File Generator** lived in a bare `<details class="solo-alt">` outside any card — `cardTitleOf()` returned null, so Theme / Assignment / Sector / Twist logged and toasted but rendered nothing. (Its `open` state was also lost on every roll, since a rerender rebuilt the element.) Fixed by giving the generator a real card (`stepCard("Alternative", "Core Case File Generator")`) with the `<details>` inside it and `st.altOpen` persisted through a `toggle` listener, so it stays open while you roll. Its Assignment button is relabelled **Core Assignment** to distinguish it from the Solo briefing's. **Guard added:** `show()` now falls back to a `LOOSE` (`"__panel"`) bucket rendered at the end of the panel, so a roll fired from outside any card can never again vanish into a toast; the per-tab clear counts it. | User: "Case generator? Results not shown" | `npm test` → **114 pass / 0 fail**. New sweep check clicks **every** 🎲 button on Solo Case/Shift/Scene and GM Prep/Play/Wrap and asserts a result slot exists in that button's own card (a plain slot count would miss the failure once a card's 3-entry history is full). Headless at 360px: all four Core-generator rolls now render inline and the details stays open; zero console errors. | brp-v43 |
| 2026-07-28 | **The ⚡ full-briefing buttons showed no result either (user-reported).** Both "⚡ Generate full briefing" (Solo) and "⚡ Full Case Briefing" (GM) wrote their block to the notes and logged it, but never called `show()`, so nothing appeared in the card — the same class of gap as the Core generator, missed because the new sweep check only clicked 🎲 buttons. Both now render the briefing inline (Assignment/Relevance/Complication/Hook, and Theme/Sector/Assignment/Twist) while still writing the full block to the notes; `show()` takes `skipAutoPin` so auto-pin does not add a duplicate one-liner underneath the block it just wrote. The GM version also loses its "are you sure?" confirm — it is a generator, and the result is now visible and dismissable. **Sweep check widened to ⚡ as well as 🎲**, which is what would have caught this. | User: "Generate full briefing still doesn't show results in panel" | `npm test` → **114 pass / 0 fail** (the notes-order check now pins from the Relevance slot, since the briefing's own slot shares that card). Headless: both briefing buttons render a four-part inline result and still append the block to the notes. | brp-v44 |
| 2026-07-28 | **"Still doesn't show" — the real cause: the page scrolled away from the result.** The briefing WAS rendering (verified in the DOM), but `router.js` `render()` called `window.scrollTo(0, 0)` on *every* render, and every roll re-renders the screen. So pressing a button halfway down the long Case panel snapped the view back to the top while the new slot sat ~1900px down — invisible, and indistinguishable from "nothing happened". This also made the earlier two reports look unfixed. Fixes: (a) the router now only jumps to the top when the **route** changes, and restores `scrollY` on an in-place re-render — this helps the sheet and combat tracker too; (b) Solo/GM remember the card that just rolled (`freshResult`) and `scrollIntoView({block:"nearest"})` its newest slot after paint; (c) switching a sub-tab explicitly scrolls to the top, since that IS a new view. | User: "Still doesn't show." | `npm test` → **115 pass / 0 fail**. New check scrolls to the ⚡ button on a long panel (scrollY 1192), clicks, and asserts the page keeps its place (1421, not 0) and the new slot is inside the viewport, then that a tab switch still lands at the top. | brp-v45 |
| 2026-07-28 | **Sheet and combat rolls now write themselves into the solo Case Notes (user request).** In solo play nobody else is keeping the record, but only oracle rolls could reach the notes — a Firearms roll or a tracker attack lived in the roll log and had to be pinned by hand. `RollLog.add()` (the single funnel every roll passes through) now mirrors entries whose `source` is `sheet` or `combat` — which covers skill rolls, pushes, opposed rolls, attacks, crits, procedural rolls and chase obstacles — into `brp:solo.scratchpad` as `• [Character · Label] outcome`, but only while `Settings.solo()` is on. Oracle rolls (`source: "solo"`/`"gm"`) are deliberately excluded: those screens already decide what goes in, via pin or the auto-pin chip. To keep the layering clean, the pure `appendToNotes()` moved from `ui.js` to `core.js` (`ui.js` re-exports it, so the note-writing screens keep one import surface) — `store.js` may now import it without depending on the UI layer. | User: "Rolls from character sheet should appear in solo case notes when solo is activated." | `npm test` → **116 pass / 0 fail**. New check rolls Firearms on the sheet with Solo on and finds `• [<name> · Firearms] …` appended after the existing notes; asserts an oracle roll is NOT double-written; asserts nothing is written at all with Solo off. | brp-v46 |
...
| 2026-07-29 | **Case Board walkthrough (user request).** The feature shipped with per-card "How to use this" notes, but no start-to-finish guide. `src/tutorial.js` gains a **5th panel, Case Board** — `Setup · Solo · Case Board · At the Table · Cheat Sheet` — holding: what the aid is and that it is **not canon**; a **10-step walkthrough** (turn Solo on → open a case → first boxes → interpret and pin → connect what you can justify → earn checks on the sheet → spend one → watch the connection counts → promote to a hypothesis → housekeeping); the **Discovery Check outcome table rendered from `DISCOVERY_OUTCOMES`**, band for band; how to read each outcome (including the degrade rule); a **rules-at-a-glance** card; a **worked Shift** end to end; and habits. Per §10.2 every number comes from `data-house.js` — box cap, clincher threshold, D100, the die ladder, and the earning skills (via `skillName`) — so correcting the aid corrects its documentation. The Solo panel's summary card now points at the guide instead of duplicating it, and both cross-panel buttons work in either direction. The board itself carries **📖 Step-by-step guide**, which opens the tutorial *on that panel*: the tutorial's remembered-panel key moved to `core.js` as `TUTORIAL_KEY` and `solo.js` passes the board an `openGuide()` (no new import in `board.js`, so no router cycle). **One bug caught by driving it:** the import edit for `data-house.js` silently no-op'd against a stale anchor, so the panel threw `H is not defined` and rendered blank — found on the first headless run, not by reading the diff. | User: "Create a tutorial on how to use this feature. Step by step guide." | `npm test` → **132 pass / 0 fail**. The new check asserts the five pills, that the walkthrough's steps are numbered in order with no gaps, that the rendered outcome bands equal `DISCOVERY_OUTCOMES` exactly, that the cap/threshold/dice text matches the live data, that nothing leaks `undefined`/`NaN`, both panel-switch buttons, zero overflow at 390px, and that the board's guide button lands on the Case Board panel even when the tutorial was last left on Setup. | brp-v51 |
| 2026-07-29 | **Countdown Timer reset left the "done this Shift" marker standing (user-reported).** `✕ Reset` set `st.timerDie` back but never cleared `shiftFlags.countdown`, so the card kept showing **✓ done this Shift** after a reset and the next roll asked "already done this Shift — roll again?". A reset is an **undo**, so it now clears the flag, drops that card's stale result slot, and returns the die to `ESCALATION_STEPS[0]`. The button label also read a hardcoded `"D6"` (§10.2) and now names the first step of the ladder from `data-solo.js`. | User: "The reset timer for shift don't work. Still shows 'done this shift' when I reset" | `npm test` → **133 pass / 0 fail**. The new check drives it through the real UI: forces a miss (die escalates D8→D10, chip appears, flag true, result slot present), presses Reset, and asserts the chip, the flag, the slot and the die are all back — then rolls again and asserts no confirm prompt appears. Also pins that the label is built from the data. | brp-v52 |
| 2026-07-29 | **Solo notes: one reset that means it (owner request).** Removed **✕ Clear notes** — with two buttons it was never clear which one you wanted, and the narrow one is the less useful. **⟲ Start a fresh case** now clears the whole case rather than most of it: it already wiped notes/oracle log/leads/checklists/timer, and now also clears **`st.results`** (the per-card inline result slots on *every* tab — the gap behind "all roll results"), the **global `RollLog`** (owner ruling: the sheet's and Home's roll-log cards read empty afterwards too), `Combat` and `Chase` (a fight or chase left over from the closed case), the Case Board, and the Shift counter + once-per-Shift markers; it then opens on the **Case** tab. It **never touches a character** — Health, Resolve, points, inventory, injuries, the Downtime cadence and journal entries are all sheet state — and preferences (auto-pin, log scope) survive as they are not case data. The confirm dialog now states both halves of that. GM-screen state is out of scope: it is not a solo tab. | Owner: "Remove the 'clear notes'… 'Start a fresh new case' — clear everything in all the solo tabs… all roll results… all pinned entries. But don't change character sheet" | `npm test` → **132 pass / 0 fail** (the old two-button test was rewritten and the narrower board-wipe check folded into it). The check seeds state on every tab plus the board, an active fight, an active chase, a global roll-log entry and a character with real data, then asserts the Clear-notes button is gone, all fourteen pieces of case state are cleared, and Health, Promotion Points, `shiftsSinceDowntime` and the journal are untouched. | brp-v53 |
| 2026-07-29 | **Solo flow audit (§11) — 8 findings, none fixed yet.** Drove a full solo case headlessly (390px) and measured the loop rather than reading it. **Verified seamless:** the pill chain Case→Shift→Scene→Leads→Wrap→Shift is complete with exactly one primary next-step per panel; the panel choice survives navigation and reload; sheet and tracker rolls auto-mirror into the case notes; End the Shift / Take Downtime drive the real character; once-per-Shift actions soft-gate; no panel dead-ends or errors with no character present; zero console errors across the loop. **Findings (work-list):** (1) every skill roll leaves the screen — Scene step 4 ends in "Open sheet to roll →", and returning resets scroll 1893→0, although `openSkillRoll(ch,key,onDone)` is a standalone modal Solo could open in place; (2) none of the three outbound jumps (sheet ×2, combat) has a return path — bottom nav only; (3) the Hypothesis Check computes its ±PP and never applies it (forced crit: result "+5 PP", character 0→0) while the next tab's Shift buttons do drive the character; (4) the award checklists count ticks but never total or apply them ("move the totals onto your sheet"), though Humanity = ticks and PP = gains − losses are fully derivable; (5) vitals, the Shift number and the Downtime-limit warning render on the **Shift** tab only — not on Scene, where damage and stress actually land; (6) a banked Discovery Check is invisible outside the Board tab, and the sheet's earn button names Solo ▸ Board without offering a jump; (7) a fired Countdown Event dead-ends in prose with no "frame a scene about it" step; (8) Notes is the only panel with no next-step. Following only the primary buttons skips the Board — intentional, since a house aid must not sit in the canon chain. | User: "Is there a seamless flow of gameplay for solo? Audit." | Headless drive of all 7 panels with and without a character, the full primary-button chain, all three outbound jumps, a forced Hypothesis crit, and status visibility measured per panel. No code changed. | n/a (audit only) |
| 2026-07-29 | **Solo flow: all 8 audit findings fixed.** (1) **Scene rolls in place** — a skill select + **⚄ Roll · ⚔ Attack · ⚖ Opposed** on the "Roll it out" card call `openSkillRoll`/`openWeaponPicker`/`openOpposedSkillRoll` directly, so the most-repeated action in solo play no longer costs a round trip to the sheet and your place on the page (the last skill rolled is remembered in `st.lastSkill`). (2) **Return paths** — the sheet and the combat tracker each carry **← Back to the solo case** while Solo Mode is on. (3) **The Hypothesis Check pays out** — it computed ±PP and never applied it; a one-tap confirm ("did that end the case?", which is the book's own condition) now applies it via a new `applyPoints()` floored at 0. (4) **The award checklists total and apply** — the card shows `Humanity +2 · Promotion +2 −1 = +1` and an **✔ Apply to \<name\>** button writes both and clears the lists; it no longer says "move the totals onto your sheet". (5+6) **One status strip above the sub-nav on every panel** — name, ♥/◈ against their maxima, Shift number, Downtime cadence (warning-coloured at the limit), the timer die, and a tappable **🔍 n** when Discovery Checks are banked; previously vitals and the Downtime warning existed on the Shift tab only, which is not where damage lands. The sheet's earn button also gained **Spend it on the Board →** (`SOLO_KEY` moved into `core.js` so `roller.js` can target a solo tab). (7) **A fired Countdown Event follows you** — held as `st.pendingEvent`, it renames the Shift panel's next-step to "The event interrupts — play it out →" and opens Scene with an **Interruption** card carrying the event text, a pin, and ✓ Played it out; ending a Shift or starting a fresh case clears it. (8) **Notes gained a next-step** back into the loop. | User: "Fix everything" after the solo-flow audit | `npm test` → **133 pass / 0 fail**. One new check drives all eight through the real UI at 390px: the strip on all 7 panels with its numbers, the 13-skill select + three roll buttons with the hash never leaving `#solo`, the earn→Board jump landing on the Board tab, both back-links, a forced crit paying +5 PP on the character, the awards line and its application (+1 PP / +2 Humanity, lists cleared), the pending event through trigger → renamed next-step → Interruption card → cleared, the Notes next-step, and zero overflow. Also fixed a pre-existing race in `update.test.mjs`: it asserted against the old execution context while the update's reload was in flight ("Execution context was destroyed") — it now waits for the navigation and a complete document first, and passed three runs in a row. | brp-v54 |
| 2026-07-29 | **Newcomer audit (user: "how idiot proof is it?") — fixes applied across every screen.** Drove the app from a wiped install as someone who has never read the rulebook or played a solo RPG. **Findings:** (1) the cold Home screen was six equal tiles with no starting point, and **Solo Mode is off by default**, so a solo player never discovered the mode existed; (2) the vocabulary the whole app speaks — push, bane, Broken, Shift, Downtime, oracle, hypothesis, Base Dice, prey/pursuer — was defined nowhere; (3) the wizard demanded ten screens of decisions with no basis for making them; (4) the character sheet is thirteen sections deep and explained none of them (Solo/GM had gained "How to use this" notes, the sheet and tracker had not); (5) the combat tracker and the chase card assumed you knew what an initiative card or a "prey" was. **Fixes:** a **first-run "Start here" card** on Home (read the walkthrough → turn Solo Mode on in one tap, which surfaces the Solo tab immediately via `navigate()` → create a character), self-retiring once a character exists and dismissable; a **`GLOSSARY`** of 27 terms in `data.js`, indexed first in the Rules Library and open by default, so searching "push" explains pushing instead of returning a weapon; **⚄ Roll me a whole Blade Runner** in the wizard (`quickBuild()` rolls nature/archetype/years/attributes/skills/specialties/memory/relationship/identity, legal by construction — key attribute B+, key skills C+, exact budgets, Replicant STR/AGI bonus — and lands on Review so nothing saves behind your back); **"How to use this" on all 13 sheet sections**, on the combat tracker (the five presses of a fight, in order) and on the chase card; and plain-language Settings toggle copy ("Playing on your own, with no one running the game?"). | User asked for a careful audit of how usable the app is with no prior knowledge, applied to all functions | `npm test` → **136 pass / 0 fail** (3 new checks: the cold-start path end to end including the Solo tab appearing and the rolled character passing both creation validators; guidance present on every sheet section, the tracker and the chase card; the glossary leading the Rules Library with a usable definition of "Push"). Drove a fresh install at 390px: Home → toggle → wizard → quick build → Create → sheet, zero console errors, no overflow. | brp-v55 |
| 2026-07-29 | **"What is this?" — the tutorial now starts before the rules (user request, closing the last newcomer gap).** Every other panel assumed the reader knows what "narrate a scene" means; someone who has never played a roleplaying game had nothing to stand on. New first panel in `tutorial.js` (`panelBasics`), and the **default panel for a fresh install**: *What a roleplaying game is* (you play one person, you describe rather than move pieces, dice settle only what is uncertain, nobody wins, it takes as long as it takes) · *What this app does for you* (sheet, dice, and — solo — the referee's job; it will not imagine the scene) · *One turn of play, start to finish* (a six-beat worked example: picture the room → say what you do → roll only if in doubt → read the dice → say what it looks like → repeat) · *If you are playing on your own* (you are both sides of the table; ask the dice only what you have not already decided; say yes to what you roll; write it down), ending with buttons into Setup and the solo loop. Deliberately carries **no rules vocabulary at all** — the glossary and cheat sheet pick that up one panel later. | User: "Yes fix that" — the app never explained what a roleplaying game is | `npm test` → **137 pass / 0 fail**. The new check asserts the panel leads the sub-nav and is the default on a fresh install, that its four cards are in order, that it hands off to Setup, and that none of the six jargon terms it exists to precede ("Base Dice", "Resolve", "Broken", "Countdown", "Hypothesis", "Promotion Point") appear anywhere in it. Drove a wiped install at 390px: tutorial opens on the panel, no overflow, zero console errors. | brp-v56 |
| 2026-07-29 | **Solo case creation was missing tables the book names (user-reported).** The Solo book gives four ways to open a case; two of them point at tables that were only reachable from the **GM screen**, which a solo player never turns on — so the generator stopped short. Added to Solo ▸ **Case**: (a) **Case Table 3 (Main NPCs)** in the Core Case File Generator — 🎲 Main NPC and ⚡ Full cast (D3+3), the same rolls `gm.js` has, because "Use the Case File Generator" cites Tables 1–4 and the app only offered 1, 2, 4 and 6; (b) a new **"Flesh out the case"** card for the book's "seek inspiration" method — ⚡ Someone involved (sphere + trait + skill level + human/Replicant), 🎲 Where it starts (Location), 🎲 Cipher; (c) a collapsed **"More Core case tables"** inside it — Clue (Ch09 Table 5), Sector location (Table 4 detail, with a sector picker), Final Confrontation (Table 7) and Mood (Table 8), all previously GM-only. Open states persist (`st.moreOpen`). No new data: every table already existed in `data-solo.js`/`data-gm.js` — this is reach, not extraction. | User: "some parts missing from the solo case creation process. Some tables were not rolled" | `npm test` → **138 pass / 0 fail**. The new check pins the full set the book names (briefing ×4 + origin, Core Theme/Assignment/Main NPCs/Sector/Twist, and the inspiration tools), then clicks all **19** rolls on the tab and asserts each renders a result in its own card with no `null`/`undefined` holes. Drove it at 390px: 19/19 render, no overflow, zero console errors. | brp-v57 |
| 2026-07-29 | **Whole-app reachability audit — data the player could not get at (user request).** Diffed every `export const` in the five data files against every reader in `src/`. **Eight tables had no reader at all:** `ATTR_LEVEL_DESC` / `SKILL_LEVEL_DESC` (the plain-language grades — the sheet showed only "C d8"), `BLAST_POWER` (explosives and vehicle weapons are rated by it), `TIME_UNITS` (how long a Round and a Shift are), `NPC_BUILD` (how to stat someone on the fly), `NPC_SKILL_DEFAULT`, `DISTINCTION_THRESHOLD` (while `gm.js` **hardcoded "Five or more"** — §10.2), and `AVAILABILITY` (a flat list superseded by `AVAILABILITY_TIERS`). **Fixes:** the sheet now reads "C d8 · Average" and lists the skill grades; the Rules Library gained Blast Power, the time scale, and three "Building an NPC" entries; the GM Prep panel gained **📋 How to stat an NPC**; Solo's NPC card names the default proficiency; the awards card reads the threshold from the data; the duplicate `AVAILABILITY` was deleted. **Also closed a reach gap the scan cannot see:** the whole Solo Mode oracle was searchable nowhere — the Rules Library now indexes a **Solo Mode** group (42 entries: Scene Check, Question Check, Critical Success, Scene Categories, NPC skill/tactics, the Countdown Timer, the Hypothesis Check and the four ways to open a case), so a player can look up "Scene Check" without turning Solo Mode on. | User: "audit for any missing elements not present for the user to use" | `npm test` → **139 pass / 0 fail**, including a new unit check that fails if ANY `export const` in `data*.js` has no reader in `src/` — the guard that keeps extraction and UI in step. Drove it at 390px: attribute rows read "C d8 · Average", the five searches (blast power / scene check / building an npc / one round / hypothesis check) all hit, the GM NPC-build card renders; zero console errors. **Still open, reported not fixed:** vehicles are Rules-Library-only — the chase tracker never asks which vehicle you are in, so Maneuverability, Hull and Armor and `VEHICLE_WEAPONS` have no play surface; and an Explosive Charge cannot be placed at a chosen Blast Power. | brp-v58 |
| 2026-07-29 | **The last two unreachable mechanics: vehicles in a chase, and placed explosives.** (a) **Vehicles were reference-only** — `VEHICLES` and `VEHICLE_WEAPONS` rendered in the Rules Library and nowhere else, so a car or Spinner chase ran on nothing but the range track. A non-foot chase now carries a **vehicle per side**: pick it, and the card shows Maneuverability / Hull / Armor, gives Hull steppers, rolls **🎲 Driving at that vehicle's Maneuverability** (`openSkillRoll` gained a `maneuver` override instead of always defaulting to C), and offers **⚔ Vehicle weapon** — a FIREARMS roll whose damage (weapon + 1 per extra success) lands on the *other* side's Hull, with a wreck notice at zero. State: `vehicles{prey,pursuer}` + `hull{prey,pursuer}` in `brp:chase` (§7). (b) **Placed explosives could not be used at all** — the weapon picker listed only `thrown` explosives, and an Explosive Charge carries the damage *range* "2–4" rather than a number, so even reaching one fell through to "special effect". Added a **Placed charges** group and `resolveBlastPower()`: choosing A/B/C resolves damage and Crit Die from `BLAST_POWER` before the attack rolls, on both the sheet and the tracker. | User: "fix everything" — the two items left open by the reachability audit | `npm test` → **141 pass / 0 fail**. Two new checks drive it: a ground chase where a Civilian Ground Car and a Spinner show their real stats, the Spinner's Driving roll reads *Maneuverability d10*, and a forced-max Autocannon burst takes the pursuer's Hull to 0/4; and an Explosive Charge picked from **Placed charges**, resolved at Blast Power A (Damage 4 · Crit d12) and rolled through to damage. | brp-v59 |
| 2026-08-29 | **Converge pass (audit prompt A).** Ran `npm test`, fixed, repeated; stopped on two consecutive green runs with no source change. **One finding, and it was the suite lying:** the run reported `pass 78 / fail 0` while **63 checks silently skipped** — `node_modules` had been reclaimed with the container, so `playwright-core` was missing and every browser-driven check took the graceful-skip path. Node's runner exits 0 on skips, so the whole browser layer read as a pass. Root cause: a skip is indistinguishable from a pass in TAP output. Fixed by `announceSkip()` in the new `tests/harness.mjs` — each browser suite now prints `!! NOT RUN — <reason>` plus the count of checks that did not execute, and §10.10 makes that a standing rule. | Audit prompt A | `npm test` twice: **141 pass / 0 fail / 0 skipped**, no source change between. Skip banner verified by forcing `CHROME_PATH=/nonexistent`. | n/a |
| 2026-08-29 | **Reachability lens (audit prompt C) — 12 + 13 + 6 + 5 findings, all closed.** New `tests/reachability.test.mjs` drives the real app over **25 surfaces** (every route plus every solo/GM/tutorial panel), clicking every visible button with localStorage snapshotted and restored around each click, and reports **every** finding per run. **R1 (12):** ten exports were reachable from nothing — `APP_NAME`, `clamp`, `rollDice`, `totalSuccesses`, `totalBanes` (`core.js`), `rollOnArray`, `dieForLevel`, `isBetterOrEqual`, `valueLevel` (`rules.js`) and `resultModal` (`ui.js`, orphaned when the chase moved to inline result slots); five of them were referenced *only by their own unit tests* — implemented, tested, wired to nothing. All deleted, tests updated. **R2 (13):** `flavorField` put its `<label>` beside the input instead of associating it, so **six identity/notes fields on the sheet had no accessible name at all** (now `for`/`id` + `aria-label`); the portrait upload took its name from the 🕵 emoji inside its own label; the solo pills and the Cipher button were named by bare jargon — `segmentNav` now takes a `hint` and `btnNamed()` gives a spoken name that still contains the visible label. **R4 (6):** empty states that named no control — the roll log (now per-screen via `rollLogCard({ emptyHint })`), the combat tracker, the journal, specialties and critical injuries all now quote a control that is on that screen. **R7 (1):** finishing the wizard navigated to **Home**, so a cold start never landed on the thing it had just built — now `navigate("sheet")`; the quick-build toast also named a button ("Finish") that does not exist ("Create Blade Runner"). **Lens corrections found while running it:** the static check could not see `$`/`$$` (word boundaries do not apply to `$`), R4's capture stopped at the first full stop — cutting off the sentence that names the control — and R9 demanded a default action from 60-button pickers. | Audit prompt C | `npm test` → **151 pass / 0 fail**. The lens runs to zero twice and prints its coverage boundary (25 surfaces, R1–R9, and eight things it deliberately does not see) on every clean run. | brp-v55 |
| 2026-08-29 | **A case now has a beginning, a middle and an end (owner report: "i still dont know how to start playing… sustaining play and ending the game well").** The third one was not a documentation gap — the app had **no ending**: nothing anywhere closed a case, the Wrap tab ended a *Shift* and looped back to the streets, and the only thing that finished a case was **⟲ Start a fresh case**, which deleted it. So ending a case and destroying its record were the same action. **Beginning:** with no case open the Case tab leads with a three-step on-ramp — **⚡ Open a case** rolls the full briefing, asks for a name, records the assignment, and drops you on the Shift tab (or **✍ Open a blank case** when you already know the case). **Middle:** with a case open the tab shows **Case #N — title**, the assignment, the Shift reached, open leads and board size, and one *Pick up where you left off* button; the status strip now names the case instead of just counting Health. **End:** **✔ Close the case** (on Case and on Wrap) asks who did it and how it ended, computes the Shifts taken and the Promotion/Humanity earned by diffing the character against `openStats` recorded at opening, writes a dated summary into the notes, and files it. **Closed cases are kept in their own store (`brp:cases`, §7) that "Start a fresh case" never touches** — a campaign now leaves a record, and an old file is the seed for the book's "follow a thread" method. Tutorial gains **A case, start to finish** — the three moments, including what "the case is over" actually means (a Hypothesis Check settles the question; a dead trail is also an ending, filed unsolved). | Owner could not start, sustain, or end a game | `npm test` → **152 pass / 0 fail**. The new check drives the whole lifecycle: the three-step on-ramp, opening (numbered, named, assignment kept, briefing in the notes, landing on Shift), resuming (the card names the case, the Shift and the open leads), closing (culprit prefilled from the leads, 4 Shifts, +5 PP from a 2→7 swing, summary in the notes), and that a fresh-case wipe leaves the filed case standing. Two existing sweeps were widened: a generator that opens a dialog before it can finish now counts as having shown a result. | brp-v56 |\n
| 2026-08-31 | **Guided play — the app now plays the game with you (owner, twice: "i still dont know how to start playing… needs to be idiot proof… just directly play straight from the app").** The tabs were a toolbox; nothing ever said *what to do*. New `src/play.js` + a **▶ Play** first pill, now the default Solo panel: one card at a time, plain English, two to four concrete choices, no tables and no jargon. It drives the official machinery — three real places off the Location tables, the Countdown check narrated as you travel, a Scene Check for the danger, genuine skill rolls with the push offered in plain words and its cost applied, Imagining Clues for finds, **Case Table 3 for named people** (a suspect is "Nombeko Koslovski — Politician, argumentative", not "Youthful Warfare contact"), and the book's **Hypothesis Check** when you accuse: a suspect's rating starts at `HYPOTHESIS.newRating` and upgrades one step per piece of evidence, which is the printed procedure. Everything found is written into the notes as it happens; solving pays out and files the case. It holds **no rules and no state of its own** (`st.play` is just which question is on screen) so the other tabs still work mid-case. **Home is now one press** — ▶ Just start playing turns Solo Mode on, routes to the wizard if there is no character, else opens Play; the old read-first three-step setup is demoted behind a collapsed "I'd rather set it up myself", because the report was from someone too lazy to read it. `rollMainNpc` lifted to module scope in `solo.js` so both screens share it. **Bug found in build — the third of its kind:** `rollPlaces`/`poolFor`/`upgrade` were `const` arrows declared below the stage dispatch that calls them, so the panel hit a TDZ and rendered blank; all are now hoisted `function` declarations, as the earlier `doneChip` and `named()` faults should have taught. **Lens correction:** the reachability cold-start clicked a control inside a collapsed `<details>`, so its hook timed out — it now requires `:visible`. | Owner could not start playing after two rounds of documentation | `npm test` → **153 pass / 0 fail / 0 skipped**, reachability lens clean over 26 surfaces. The new check drives a whole case through the guided panel: one choice with no case, three places offered, arrival in one press, four concrete actions, a find written to the notes unprompted, a named suspect, a failed roll offering the push and Resolve actually dropping, the accusation paying Promotion Points, and the closed case filed with its culprit. | brp-v60 |\n
| 2026-09-08 | **Installed the `solo-rpg-playtester` agent** (owner-supplied) at `.claude/agents/solo-rpg-playtester.md`, checked into the repo so it travels with the project rather than living in one machine's user config. It is a **playtest** lens, not a code lens: it learns the game from the app itself (rules data → in-app tutorial → spec, writing a six-heading brief first), builds a label-driven Playwright driver whose commands persist state between invocations, then either **AUDITs** (seeded page RNG *plus* a separate driver PRNG for branch choice, ≥3 seeds, non-zero exit on a stall) or **PLAYs** (decides in the fiction, writes prose each beat, finishes properly so end-of-session economy fires). Its finding classes are ranked by how disabling they are — a stall, a fiction dead end, a described capability with no control, a silent state change, a lie in the record — and it is explicitly barred from reporting code quality, which the existing suites own. No app file changed, so no cache bump. | Owner supplied the agent and asked for it to be installed | `npm test` → **153 pass / 0 fail / 0 skipped**; the file is frontmatter-valid (`name`, `description`, `tools`) and 211 lines copied verbatim. The first run of this pass reported `pass 78 / skipped 75` — `node_modules` had been reclaimed with the container again, and the §10.10 `!! NOT RUN` banners are what made that visible rather than passing as green; `npm install` restored it. | n/a (agent definition — nothing shipped) |
| 2026-09-09 | **Exhaustive playtest audit (`solo-rpg-playtester`, AUDIT mode, seeds 2/11/23) — 15 findings, none fixed.** Verdict: **a solo session plays start to finish on every seed with zero console errors** — cold start → quick-built Blade Runner → case → travel → rolls/pushes → clues → named suspects → accusation → case filed with its points, clicking only what the app offers (seed 23 closed on a critical success; 2 and 11 failed the accusation, lost PP and correctly continued). Findings, most disabling first: (1) **a Broken combatant cannot be attacked** — every target list filters `health > 0` (`roller.js` 857/952/1012), so ranged, full-auto spill and opposed melee all lose the target, and with every enemy down the attack dialog has no Target section yet still prints damage and a crit with nowhere to apply them; the §3.7 "damage on an already-Broken combatant forces a crit" path exists only via the card's `−` stepper. (2) **A dying NPC is invisible in the tracker** — a lethal crit renders identically to a cosmetic one, with no interval and no Death save/Stabilize controls, while the PC sheet shows both. (3) **A dead Blade Runner keeps playing** — the DECEASED banner names the wizard but offers no control, skills still roll, and Solo happily opens a new case for the corpse. (4) **`▶ Play` changes vitals with no record** — `play.js` rolls dice itself instead of through `roller.js`, so no guided roll reaches `RollLog`, and its `endShift` skips the `record()`/toast the Wrap tab uses, so passing the Downtime limit costs 1 Resolve silently. (5) **"Follow a thread" never reads `brp:cases`** — it writes a blank template while the filed cases sit in the same card. (6) **Chases ignore a wrecked vehicle** (toast only, still offers Driving) and **"Caught" fires one step past Engaged**, so the threshold the card prints does nothing. (7) **The record overstates a failed hypothesis** — `applyPoints` floors at 0 but the notes and case file still say −3. (8) The award checklists are `<details>` that collapse on every tick. (9) Case Board results render into the loose bucket, so they appear under Case/Shift/Leads/Wrap too. (10) `logRoll` runs before any push, so an unpushed failure is logged as carrying a bane. (11) The briefing is appended to the notes before the "Name the case" dialog is accepted. (12) Offline, Settings sits on "Connecting to cloud sync…" forever. (13) Two controls the app tells you to use are not buttons (the tracker's initiative `<span>`, Solo's unlabelled question-odds `<select>`). (14) The wizard ships an unnamed character — Identity's `Next ›` is enabled with every field blank. (15) Minors: a Replicant at 0 PP is never told a Baseline Test is due; no gate on searching one location 11 times; two generated suspects shared a name. **Not covered:** GM screen, Firebase/multiplayer, blow-by-blow combat (armor dice, spill, aiming consumption, ambush order), PWA/update/theme, keyboard & screen-reader passes, exhaustive data sweeps, migration/quota/two-tab. | Owner: "Run this agent to cover all the functions in the app. Be exhaustive." | Label-driven Playwright driver in the scratchpad (page `Math.random` seeded and fast-forwarded across invocations, branch choice from a separate driver PRNG, state on disk per seed); repo untouched, no app source changed. | n/a (audit only) |
| 2026-09-09 | **All 15 playtest findings fixed.** **(1) A Broken combatant can be attacked** — the three target lists in `roller.js` filtered `health > 0`, so ranged, full-auto spill and opposed melee all lost a downed adversary and the attack dialog printed damage with nowhere to put it; the filters are gone and a target chip reads "♥0 Broken". **(2) A dying NPC is legible and treatable** — a combatant card now states `lethal — <interval> save` and carries **Death save (STAMINA)** and **Stabilize (MEDICAL AID)**, driven by a new `rollCombatDeathProcedure()` that mirrors the sheet's procedure (failure marks the combatant dead and, for a PC, their sheet; a successful stabilize raises Round→Shift then ends the saves). **Found while there:** `openCombatSkillExecute` called `nextSteps(ch, …)` with no `ch` in scope — a ReferenceError on every tracker skill-roll result — now `rc.pc || null`, and `nextSteps` tolerates a null character. **(3) A dead Blade Runner is inert** — `blockIfDead()` gates all four `roller.js` entry points, guided play shows an end-of-the-line card instead of handing out cases, the status strip and the Characters roster mark the death, and the DECEASED banner carries **＋ New Blade Runner** / **Switch character**. **(4) Guided play keeps a record** — `play.js` logs every roll (skill, push, Countdown, Hypothesis) to `RollLog` as `source: "solo"`, says in the notes and a toast what a push cost, and its `endShift` now runs the Wrap tab's `record()`/toast so passing the Downtime limit is no longer a silent −1 Resolve. **(5) "Follow a thread" reads `brp:cases`** — it opens a picker of the filed cases and seeds the note with the case number, title, assignment and how it ended. **(6) Chases:** a wrecked vehicle is a state (badge, log line, Driving and Vehicle-weapon disabled), and **Caught fires at Engaged** (`distIdx <= 0`, the threshold the card itself prints) with a card offering the free attack. **(7) The record stops overstating** — `applyPoints` floors at 0 and now returns the swing it actually applied; the hypothesis note, the guided accusation, the awards line and the toast all quote that. **(8)** Award checklists remember their open state (`st.checkOpen`) so a tick no longer closes the list. **(9)** The loose result bucket is **per tab** (`__panel:<panel>`), so a Board result rolled from a dialog no longer appears under Case/Shift/Leads/Wrap; the legacy shared bucket is dropped on read. **(10)** `outcomeSummary(succ, banes, pushed)` — an unpushed 1 costs nothing, so it is no longer logged as a bane (the same in the on-screen outcome line). **(11)** The briefing is written to the notes only after the case is named, in both the guided panel and the Case tab. **(12)** `initSync` marks a failure (with a 12s timeout on the SDK import and the anonymous sign-in) and Settings says the app is offline and offers **Try again**, instead of "Connecting to cloud sync…" for ever. **(13)** The tracker's initiative card is a real `<button>` with an accessible name; the Solo question-odds `<select>` has one too. **(14)** The wizard gates Identity on a non-empty name (updating the Next button in place, so the caret is not stolen mid-word). **(15)** A Replicant at 0 Promotion Points gets a **⚠ Baseline Test due** chip in the status strip; a worked-out location demotes its four actions and promotes "Go somewhere else"; generated people re-roll a duplicate name. | Owner: "Fix all" | `npm test` → **165 pass / 0 fail / 0 skipped** (78 unit + 87 browser). Twelve new checks, one per finding: a Broken target offered by name, a dying NPC's save controls, a dead character rolling nothing and being marked everywhere, a guided roll reaching the roll log with no phantom bane, the thread picker naming a filed case, a wreck that survives a re-render with Driving disabled plus the Caught card at Engaged, a 1-PP character losing exactly 1 on a failed hypothesis, a checklist still open after a tick, a board result absent from four other tabs, a cancelled name prompt leaving no briefing, the offline account panel, the name-gated wizard, and the Baseline chip. | brp-v61 |
| 2026-09-09 | **The two bugs the fix pass turned up, closed properly.** Both were fixed inside the previous change but only one carried a regression check, and one of them existed twice. **(a)** `openCombatSkillExecute` rendered its result with `nextSteps(ch, …)` and had no `ch` in scope — a ReferenceError on **every** skill roll started from the combat tracker (the sheet's own rolls pass a real character, which is why nothing caught it). Fixed to `rc.pc || null` with `nextSteps` tolerating a null character; a new check now rolls Observation for an NPC in the tracker and asserts the result renders with no console error. **(b)** The shared `__panel` result bucket — a roll fired from a dialog has no owning card, and every panel read the same key, so the result appeared under all of them. `solo.js` was made per-tab in the same pass; **`gm.js` had the identical defect and was not touched**. It is now `__panel:<panel>` there too, with the legacy shared key dropped on read, and a check asserts a Prep result stays off Play/Fight/Wrap. | Owner: "Fix the two extra bugs found" | `npm test` → **167 pass / 0 fail / 0 skipped**. Two new checks, one per bug. | brp-v62 |
| 2026-09-09 | **All 9 findings from the PLAY-session journal fixed.** The playtester played a full case (Chaya Hoff, "Rainfall", 4 Shifts + Downtime, closed on a critical Hypothesis Check) and reported friction rather than faults; every item is closed. **(1) Guided play now feeds the surfaces it drives** — it kept the case to itself, so the Board read *"Empty board"* and Leads *"No active hypotheses"* while `▶ Play` privately tracked both, and the tester re-typed eleven boxes by hand. Each kept clue and named person now becomes a Case Board box (the clue linked to the suspect it points at), each suspect a Leads hypothesis at the panel's own rating (`playId` ties the two), a disproved accusation drops its lead instead of leaving it at full rating, and an investigative success offers **🔍 Bank a Discovery Check** — previously the Board's economy was unreachable from the default panel. **(2+5) The chase rolls the NPC's maneuver where the chase is** — the card offered a *Prey maneuver* row with no way to roll one while the deciding table sat three taps away on Solo ▸ Scene; both the chase card (Solo only) and the Scene card now roll **one side at a time**, so the pinned note can no longer claim a move the other side never made. **(3)** ▶ Get me a case **shows the briefing first** (assignment/relevance/complication/hook, with *Take the case* / *Give me a different one*) and asks for a name only on acceptance; the Case tab's prompt now carries the briefing text in the dialog. **(4)** Attack log labels named the attacker twice (`Chaya Hoff · Ranged … — Chaya Hoff`) and never the target — now `Ranged <weapon> → <target>` and `Close combat <weapon> → <defender>`, with the character column still naming the attacker. **(6)** A closed case undercounted what it paid: the `openStats` diff read zero for a case whose Humanity was spent on a skill step before closing. Schema **v5** adds `state.spent{pp,humanity}` (incremented by the specialty, skill-raise and Promotion-Point purchase flows — never by a penalty), and `closeCase` adds the spend delta back. **(7)** Declining a push left the push offer standing as the current situation — the card now says the line of enquiry came to nothing. **(8)** The creation draft is persisted to `brp:wizard` on every paint and restored (merged over a fresh draft, so a shape change cannot crash it), cleared on finish and on cancelling out of step 1 — ten screens of decisions used to vanish if a phone backgrounded the tab. **(9)** The travel card stopped telling you *"There is no wrong answer"* on Shift 4: with a suspect it names them, after Shift 1 it points at what you turned up. | Owner: "Fix all" after the PLAY session | `npm test` → **175 pass / 0 fail / 0 skipped**. Nine new checks, one per finding: the board and leads filled by guided play with the clue linked and the lead dropped on a failed accusation, the chase's per-side NPC roll and its one-column pin, the briefing on screen before any prompt, an attack log line naming the target, a case paying 5 Humanity with none left in the pocket, a refused push clearing its offer, a quick-built draft surviving a reload at step 10, and the Shift-4 travel card naming the suspect. Three existing checks moved with the code: the schema check to v5 (plus spend back-fill inside a legacy `state`), the guided-play walk through the new briefing card, and — a real fault the fix exposed — `router.js` called `renderWizard(m, { restart: true })` on **every** navigation, so a draft could not have survived a reload even with persistence; the restart is gone and the wizard clears its own draft on finish or on backing out of step 1. | brp-v63 |
| 2026-09-10 | **All 15 findings + minors from the CHORUS play session fixed.** **(1) Guided play now rolls the engine's dice** — `▶ Play` built a bare `[attributeDie, skillDie]` pool, so the panel aimed at beginners silently discarded their own character sheet; `roller.js` exports `skillPool()` (the sheet's own auto-advantage maths, Aiming consumed, notes returned) and `play.js` calls it, reporting *"Your condition counted: Cracked skull: −1"*. Its push now keeps successes as the engine does, and is not offered when nothing can re-roll. **(2) The chase knows what an NPC is** — both sides rolled the player's character, offered them the player's key memory and allowed a push the book forbids; each side is now marked **Your character / An NPC**, an NPC rolling the vehicle's Maneuverability + its `NPC_SKILL_LEVEL` dice inline, unpushable, labelled as such (`npcSide`/`npcLevel`/`npcRoll` in `brp:chase`). **(3)** Damage landing on a Broken combatant now offers **☠ Roll the forced critical injury** — the rule was announced in a toast with no control anywhere. **(4)** Death was one press with no way back: the DECEASED banner carries **↺ Undo — that was a misclick** behind a confirm. **(5)** `matchWeapon` tested the whole *"A or B"* gear string, so a fuzzy rule hit the back half and armed the Corporate Killer with a pistol from neither option; a choice is now split and matched half by half. **(6)** The opposed-melee counter-hit used `weapons[0]` — a slung rifle's *ranged* Damage — and now picks a melee weapon (or bare hands) and names it. **(7)** A combatant's own condition chips never fed their own attack; `selfCombatMods()` reads Aiming, Prone and crit-injury disadvantages from the card, and Aiming is spent by the shot. **(8)** The tracker's weapon picker gained **Thrown / explosives** and **Placed charges**, so a charge can be resolved *at* somebody. **(9)** `pushable()` now requires a die that can actually re-roll — an all-1s pool changed nothing and charged its banes again. **(10)** A lethal `shift`-interval wound owes a save every Shift and nothing ever asked: the sheet's Investigation/Downtime Shift and the Premium-purchase Shift prompt it, and Solo's Shift buttons warn and write it into the notes. **(11)** Opening a case clears the closed case's leads, once-per-Shift markers and pending event. **(12)** A case opened by the Core Case File Generator files with its rolled assignment (`st.coreAssignment`). **(13)** A once-per-Shift heal at full vitals is refused instead of burned. **(14)** *"A Shift to arrive"* now costs a real Shift on a Premium purchase, won or lost. **(15)** Full auto splits its extra successes — each is **either** +1 damage to the primary **or** one spilled elsewhere, the primary figure dropping as you assign, friendly targets marked. **Minors:** the archetype-free build no longer advertises a name-roll button it cannot render (and says ⚄, not 🎲); Scene's roll card no longer claims rolls happen elsewhere; the Board's Connect message distinguishes "nothing to connect" from "already fully connected"; the close-case control reads **✔ Close the case (solved or cold)**; Board suspects are named people off Case Table 3, not trait blobs; a dead NPC is refused as dead, not Broken; the character journal reads oldest-first like the notes and the log; awards and applied Promotion swings are written into the case notes; and blur-to-save restores focus to the element Tab was moving to. **Not fixed, deliberately:** a vehicle wrecked in a chase stays sellable in inventory — the chase's vehicle picker is scratch state, not an inventory link, and wiring them would make the chase mutate the sheet. | Owner: "Fix all" after the CHORUS session | `npm test` → **188 pass / 0 fail / 0 skipped**. Twelve new checks, one per major finding plus a minors sweep: an injured character's guided pool losing a die, an NPC chase roll at its own level that opens no player dialog and says it is never pushed, the forced-crit button, an undone death, a gear choice arming from its own options, an all-1s failure offering no push, a Shift prompting the save it owes, a new case dropping the old leads while keeping the generated assignment, a heal refused at full Resolve, a Premium buy moving the cadence, a spill taking its success off the primary's damage, and named board suspects + journal order. Three existing checks were corrected: two forced every die to a 1 and then asserted a push was offered — which is now precisely what the engine refuses — and the chase test caught a real slip in this pass, where the edit that added the who-is-driving chips replaced the vehicle `<select>`'s append instead of preceding it. | brp-v64 |
